import { Candle, Trend, BreakoutEvent } from '../types';

export class BreakoutDetector {
  // We start by assuming a Bullish trend. This will correct itself upon the first breakout.
  private currentTrend: Trend = 'BULLISH';
  
  // Track if the active pivot has already been broken to prevent duplicate breakouts
  private activeSwingHighCrossed: boolean = false;
  private activeSwingLowCrossed: boolean = false;
  
  // Keep track of the pivot levels to know when they change
  private lastEvaluatedSwingHigh: number | null = null;
  private lastEvaluatedSwingLow: number | null = null;
