...
class DeepSeekClient {
    static async analyzeEvent(symbol, event) {
        if (!env_1.config.deepseekApiKey) {
            console.warn('DEEPSEEK_API_KEY missing. Skipping AI analysis.');
            return 'AI analysis skipped due to missing API key.';
        }
        const systemPrompt = 'You are a professional Smart Money Concepts (SMC) market structure analyst.\n' +
        ...
        const userPrompt = `A confirmed ${event.direction} ${event.event} has been detected. Produce a structured market briefing.\n\n` +
            `Symbol:                 ${symbol}\n` +
